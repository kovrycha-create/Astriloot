...
// --- HELPER HOOKS & CLASSES ---

/**
 * A custom hook to manage state in localStorage, abstracting away the
 * boilerplate of reading, parsing, and writing.
...
/**
 * A simple Linear Congruential Generator (LCG) for seedable pseudo-random numbers.
 * This makes game logic deterministic for a given seed, which is crucial for
 * debugging, testing, and consistent gameplay experiences.
 */
export class SeedableRNG {
  ...
  /**
   * Generates the next pseudo-random integer between min (inclusive) and max (inclusive).
   */
...
const useGameLogic = () => {
    ...
    const rng = useMemo(() => new SeedableRNG(runSeed), [runSeed]);
    ...
    const [autoEquip, setAutoEquip] = useLocalStorage('autoEquip', false);
    const [combatSpeed, setCombatSpeed] = useLocalStorage('combatSpeed', 1);
    const [isCombatSpeedLocked, setIsCombatSpeedLocked] = useLocalStorage('isCombatSpeedLocked', false);
    const [isAchievementsVisible, setIsAchievementsVisible] = useState(false);

    // Camp & Achievements (Persisted)
    const [campState, setCampState] = useLocalStorage('campState', INITIAL_CAMP_STATE);
    const [playerAchievements, setPlayerAchievements] = useLocalStorage<PlayerAchievements>('playerAchievements', getInitialPlayerAchievements());
    ...
    const hasUnclaimedRewards = useMemo(() => Object.values(playerAchievements).some(ach => ach.unlockedTier > ach.claimedTier), [playerAchievements]);

    // Data integrity check for achievements on load
    useEffect(() => {
        const initialAchievements = getInitialPlayerAchievements();
        let needsUpdate = false;
        const updatedAchievements = { ...playerAchievements };
        for (const key in initialAchievements) {
            if (!updatedAchievements[key]) {
                updatedAchievements[key] = initialAchievements[key];
                needsUpdate = true;
            }
        }
        if (needsUpdate) {
    ...
    const generateAndSetJourney = useCallback(async (currentVictories: number, playerLevel: number, activeElixir: ActiveElixirEffect | null) => {
        try {
            ...
            const generatedNodes = generateNodes(rng, forceFirstNodePositive);
    ...
    const startNewRun = useCallback(() => {
        if (!player) return;
        const vasLossPercent = ECONOMY_TUNING.risk.death_vas_loss_percent;
        const vasAfterPenalty = Math.floor(player.vas * (1 - vasLossPercent));
        ...
        setRunSeed(Date.now()); // Generate a new seed for the new run
    ...
    const handleNextJourneyStep = useCallback(() => {
        ...
        setJourneyPath(prevPath => {
            ...
            const nextStep = calculateNextStep(rng, currentPos, mouseInfluence);
            return [...prevPath, nextStep];
        });
    }, [nodes, mouseInfluence, player, showToast, rng]);
    ...
    const handleCombatEnd = useCallback((playerWon: boolean, finalLog: CombatLogEntry[], combatStats: Partial<RunStats>) => {
        ...
        if (playerWon) {
            ...
            const essenceGained = 5 + rng.nextInt(0, 9) + (enemy?.level || player.level);
    ...
    }, [enemy, player, victories, addToHistory, runStats, runHistory, showToast, handleUnlockAchievements, updateBuffs, rng, setCampState]);
    ...
    const streamAndLogNarrative = useCallback(async (outcome: any) => {
        ...
        setCombatLog(prev => [...prev, { id: Date.now() + rng.next(), narrative: fullMessage, outcome }]);
        setCurrentNarrative('');
    }, [rng]);
    ...
    const handleCurrencyExchange = useCallback((from: 'vas' | 'ae', to: 'vas' | 'ae', amount: number, rate: number) => {
        if (!player) return;
        if (from === 'vas' && to === 'ae') {
            const cost = amount * rate;
            if (player.vas < cost) {
                showToast("Not enough Vas for this exchange.", "error");
                return;
            }
            setPlayer(p => {
                if (!p) return null;
                return { ...p, vas: p.vas - cost, essence: p.essence + amount };
            });
            showToast(`Exchanged ${cost} Vas for ${amount} Arcane Essence!`, 'success');
        }
    }, [player, showToast]);
    ...
    }, [player, campState.upgrades, showToast, setCampState]);
    ...
    }, [showToast, addToHistory, setPlayerAchievements]);
    ...
    }, [playerAchievements, showToast, addToHistory, setPlayerAchievements]);
    ...
    }, [gameState, victories, player, justReceivedTreasure, showToast, rng]);
    ...
    }, [gameState, journeyEvent?.type, player, runHistory, showToast, rng]);

    return {
        // State
        gameState, isLoading, loadingMessage, player, playerWithStats, enemy, combatLog,
        currentNarrative, victories, journeyNarrative, mapImageUrl, journeyPath, nodes,
        isJourneyPaused, itemForRitual, revealedArtifact, lastCombatResult, gearForChoice,
        journeyEvent, dilemmaResult, autoEquip, combatSpeed, isCombatSpeedLocked, campState,
        playerAchievements, hasUnclaimedRewards, isAchievementsVisible, toast,
        enemyStatusEffects,
        // RNG instance
        rng,
        // Setters and Handlers
        setPlayer, setEnemy, setPlayerHealth: (hp: number) => setPlayer(p => p ? ({ ...p, health: hp }) : null),
        setEnemyHealth: (hp: number) => setEnemy(e => e ? { ...e, health: hp } : null),
        setPlayerStatusEffects, setEnemyStatusEffects, onCombatEnd: handleCombatEnd, streamAndLogNarrative,
        handleCharacterSelect, handleNextJourneyStep, handleRitualComplete, handleArtifactRevealContinue,
        handleGearChoice, handleJourneyEventContinue, handleDilemmaResultContinue, handleEchoingCairnResolve,
        handlePurchase, handleCurrencyExchange, handleDilemmaResolve, handlePurchaseUpgrade, handleBrewElixir, handleExitCamp,
        handleApplyEnchant, handleSocketGem, handleClaimReward, handleClaimAllRewards, handleLootContinue,
        setMouseInfluence, setAutoEquip, setCombatSpeed, setIsCombatSpeedLocked,
        setIsAchievementsVisible, setToast,
        // Exported for internal component use
        updateBuffs,
        setIsJourneyPaused,
        setGameState,
    };
...
const App: React.FC = () => {
    ...
    const {
        gameState, isLoading, loadingMessage, playerWithStats, enemy, combatLog,
        currentNarrative, victories, journeyNarrative, mapImageUrl, journeyPath, nodes,
        isJourneyPaused, itemForRitual, revealedArtifact, lastCombatResult, gearForChoice,
        journeyEvent, dilemmaResult, autoEquip, combatSpeed, isCombatSpeedLocked, campState,
        playerAchievements, hasUnclaimedRewards, isAchievementsVisible, toast,
        enemyStatusEffects, rng,
        setPlayerHealth, setEnemyHealth, setPlayerStatusEffects, setEnemyStatusEffects, onCombatEnd,
        streamAndLogNarrative, handleCharacterSelect, handleNextJourneyStep,
        handleRitualComplete, handleArtifactRevealContinue, handleGearChoice,
        handleJourneyEventContinue, handleDilemmaResultContinue, handleEchoingCairnResolve,
        handlePurchase, handleCurrencyExchange, handleDilemmaResolve, handlePurchaseUpgrade, handleBrewElixir,
        handleExitCamp, handleApplyEnchant, handleSocketGem, handleClaimReward,
        handleClaimAllRewards, handleLootContinue, setMouseInfluence, setAutoEquip,
        setCombatSpeed, setIsCombatSpeedLocked, setIsAchievementsVisible, setToast
    } = game;

    const headerGlowClass = useMemo(() => {
        if (victories >= 25) return 'animate-header-glow-mythic';
        if (victories >= 10) return 'animate-header-glow-orange';
        return 'animate-header-glow-purple';
    }, [victories]);

    const containerMaxWidthClass = useMemo(() => {
        switch (gameState) {
            case GameStateEnum.CHARACTER_SELECT: return 'max-w-4xl';
            ...
            case GameStateEnum.LOOT: return 'max-w-5xl';
    ...
    const renderGameState = () => {
        ...
        case GameStateEnum.MERCHANT:
            ...
            <MerchantPhase
                ...
                onExit={() => {
                game.updateBuffs();
                game.setIsJourneyPaused(false);
                game.setGameState(GameStateEnum.JOURNEY);
                }}
        ...
        case GameStateEnum.CAMP:
            ...
            <CampPhase
                ...
                onEnterEnchanting={() => game.setGameState(GameStateEnum.ENCHANTING)}
                onEnterGemSocketing={() => game.setGameState(GameStateEnum.GEM_SOCKETING)}
            />
            );
        case GameStateEnum.ENCHANTING:
            return (
                <EnchantingPhase
                    ...
                    onExit={() => game.setGameState(GameStateEnum.CAMP)}
                />
            );
    ...
    return (
        ...
        <header className={`sticky top-0 z-40 bg-black/30 backdrop-blur-md border-b border-purple-800/50 py-3 transition-colors duration-500 ${headerGlowClass}`}>
            ...
            <div className="flex items-end gap-4">
                ...
                {game.player ? `${game.player.name}'s Journey` : 'A New Beginning'}
                </span>
            </div>
